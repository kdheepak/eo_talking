...
class Controller(HelicsAgent):
    ...
    def __init__(
        ...
        self.register_publication(
            self.talk_to_home_topic, self.talk_to_home_topic, "String"
        )
        self.register_publication(
            self.talk_to_eo_topic, self.talk_to_eo_topic, "String"
        )
    ...
    def send_bids_to_energy_orders(self):
        ...
        upcoming_schedule = {
            self.node_id: {
                k: [self.current_time, -0.04]
                for k in range(period_start, period_start + 120, 60)
            }
        }
    ...
    def listen_to_home(self):
        self.home_energy_readings[self.current_time] = self.fetch_subscription(
            self.listen_to_home_topic
        )
