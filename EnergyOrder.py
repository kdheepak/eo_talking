...
class EnergyOrders(HelicsAgent):
    def __init__(self, fed_name,message_interval=TIMESTEP,fedinitstring=fedinitstring):
        ...
        self.register_publication(self.talk_to_controllers_topic, self.talk_to_controllers_topic ,"String")
        self.register_publication(self.talk_to_market_topic, self.talk_to_market_topic,"String")
        for listen_topic in self.listen_topics.split(';'):
            self.register_subcription(listen_topic, listen_topic)
        self.register_subcription(self.listen_to_market_topic, self.listen_to_market_topic)
    ...
    def publish_approved_to_controllers(self):
        self.publish(self.talk_to_controllers_topic,'{}'.format(json.dumps(self.completed_orders)))
